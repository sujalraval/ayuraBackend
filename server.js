...
app.use(cors({
    origin: function (origin, callback) {
        ...
        // For now allow all, consider locking down in prod
...
app.use(helmet({
    ...
    // Could add contentSecurityPolicy if needed
}));
...
// This must be placed BEFORE all routes and catch-alls!
...
// === OPTIONAL: FILE TEST LISTING ROUTE ===
app.get('/debug/uploads', (req, res) => {
    const walk = (dir, prefix = '') => fs.readdirSync(dir).flatMap(file => {
        const fp = path.join(dir, file);
        if (fs.statSync(fp).isDirectory()) return walk(fp, prefix + file + '/');
        return [{
            path: prefix + file,
    ...
    try {
        ...
        res.status(500).json({ error: e.message, uploads_exists: fs.existsSync(uploadsDir) });
    }
});
...
// === CATCH-ALL for OTHER ROUTES (DO NOT HANDLE /uploads) ===
app.use('*', (req, res) => {
    // Only match if not /uploads
    if (req.originalUrl.startsWith('/uploads/')) {
        return res.status(404).json({ success: false, message: 'File not found' });
    }
    // Otherwise, handle as generic 404
    res.status(404).json({ success: false, message: `Route ${req.originalUrl} not found` });
});

// === ERROR HANDLING ===
app.use((err, req, res, next) => {
    console.error('Error:', err.message, req.originalUrl);
...
app.listen(PORT, () => {
    console.log(`
  🚀 Server running @ ${PORT}
  📁 Uploads Directory: ${uploadsDir} (exists: ${fs.existsSync(uploadsDir)})
